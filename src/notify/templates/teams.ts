  ...
  const rows =
    lineUp.players.length === 0
      ...
      : `<ul style="margin:0 0 12px; padding-left:20px; font-size:14px; line-height:1.6; color:#1c1b19;">${lineUp.players
          ...
          .join("")}</ul>`;
  return `<h3 style="margin:0 0 6px; font-size:15px; color:#1c1b19;">${escapeHtml(lineUp.name)}</h3>${rows}`;
}
...
<p style="margin:0 0 16px; font-size:15px; line-height:1.5; color:#4a4741;">${escapeHtml(whenLocal)}</p>

<p style="margin:0 0 20px; font-size:17px; line-height:1.5; color:#1f6f4a; font-weight:700;">${escapeHtml(lead)}</p>
