...
/**
 * The page's own sections, in the order they appear.
 *
 * Kept as data so the contents list and the headings themselves cannot come
 * to disagree: `heading()` below is the only thing that writes an `<h2>`, and
 * it reads its text from here. A section added to the page without an entry
 * here throws on the first render rather than quietly appearing with no line
 * in the contents — see the guard inside `heading`.
 */
const SECTIONS: readonly { id: string; title: string }[] = [
  { id: "who", title: "Who holds it" },
  { id: "held", title: "What is held, and why" },
  { id: "lawful", title: "Why we are allowed to" },
  { id: "shared", title: "Who else sees it" },
  { id: "kept", title: "How long it is kept" },
  { id: "rights", title: "What you can do" },
  { id: "limits", title: "What deleting your data does not reach" },
  { id: "cookies", title: "Cookies" },
];

/** One section heading, with the anchor the contents list points at. */
function heading(id: string): string {
  const section = SECTIONS.find((candidate) => candidate.id === id);
  // A lookup that can miss, in the shape CLAUDE.md warns about: escapeHtml
  // throws on undefined and would 500 the one page a person reads before
  // deciding whether to trust us with an address. Naming the id is what makes
  // the mistake findable.
  if (section === undefined) throw new Error(`no privacy section with id "${id}"`);
  return `<h2 id="${escapeHtml(section.id)}">${escapeHtml(section.title)}</h2>`;
}

/**
 * Where the page goes, before it goes there.
 *
 * This is eight sections of prose — by some distance the longest page in the
 * product, and the only one nobody arrives at for pleasure. A reader here
 * almost always wants one specific thing (what is shared, how to delete it),
 * and until M55 the only way to find it was to read everything above it.
 */
function contents(): string {
  const links = SECTIONS.map(
    (section) => `<a href="#${escapeHtml(section.id)}">${escapeHtml(section.title)}</a>`,
  ).join(" · ");
  return `<nav class="contents" aria-label="On this page">${links}</nav>`;
}
...
export function renderPrivacyPage(): string {
  ...
  // To the holding page, and not to the account page or anywhere else inside
  // the app. This page takes no `nav` — it is public, and most of its readers
  // hold no session — so it carried no header and was the one page in the
  // product with no way onward at all. The destination has to be one every
  // reader can actually reach, which rules out anything behind sign-in.
  //
  // A TS comment and not an HTML one: an HTML comment is page content, and
  // this codebase has tripped its own "these words are absent" assertions on
  // comment text three times already.
  const backLink = `<p class="back-link"><a href="/">Back to Make The Team</a></p>`;
  ...
  const body = `
    ...
    <p><strong>An organiser of a game you're in can send you a message.</strong> It is not only this product that emails you — an organiser can write a short message to the squad, and it reaches you by email, and by push too if you have registered a device for it.</p>
