...
export function renderAdminAllowlistPage(params: AdminAllowlistPageParams): string {
  const { secretEntries, tableEntries, error } = params;
  ...
  const list =
    items.length === 0
      ? `<p>Nobody is on the allow list. Only invited players can sign in.</p>`
  ...
  return layout({
    ...
    body: `
      ...
      <p>People on this list can create an account without being invited to a game first. Anyone already invited to a squad can sign in regardless — this list is only for letting someone in ahead of their first invite.</p>
      ${list}
