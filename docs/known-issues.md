...
| `EMAIL_FROM` is declared non-optional in `Bindings` but is absent from `wrangler.jsonc` vars. Nothing constructed `ResendNotifier` from `env` while this was written, so it was harmless at the time — that is no longer true now that M3 wires a real send path. Without it the `from` field is silently `undefined` on every real send. | `src/env.ts`, `wrangler.jsonc` | M3 deploy (task 17), before `NOTIFIER=resend` is ever set in production |
...
| The test-only `fetch` spy in the Resend notifier tests throws on an unexpected call, but `sendBatch` catches everything, so an unmocked call becomes a `{ok:false}` result rather than a failed test. Real-network safety holds; detection of a forgotten mock does not. | Test-suite robustness only, not a production risk. |
| `vitest.config.ts` sets no `outboundService`, so there is no repo-wide backstop stopping a test suite from reaching the real network — each suite is safe only via its own spy. | Same category as above. |
...
| `NO_RECIPIENT_REASON` is handled as retryable by the sweep's dedupe-log cleanup, but is actually unreachable from that caller — guests are filtered out before a `Message` is ever built (BR-32). If it ever did fire for a genuinely permanent reason, it would retry every hour forever instead of surfacing as `failed`. | Currently dead code path; revisit if a new caller of the retry logic is added. |
