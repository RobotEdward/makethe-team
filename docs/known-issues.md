...
| **A push subscription's `endpoint` is any `https:` URL, deliberately — not an allowlist of push services.** An external security review (4 September 2026) rated this a critical SSRF: a signed-in player registers an internal or attacker-controlled URL and the Worker POSTs to it at notification time. Checked against source and rejected at that severity. The Worker never reads the response body (`pushToDevice` in `src/notify/push-notifier.ts` uses only the status code), the payload is encrypted to the subscriber's own `p256dh` key and carries only that subscriber's own notifications, the VAPID token's audience is the target origin so it is useless elsewhere, and Cloudflare Workers have no metadata service and cannot reach private or link-local addresses at all. What remains is a blind request oracle: an allowlisted, signed-in player can make the Worker POST an opaque blob to any public `https:` URL and, via the account page's Test button, learn whether it answered 2xx. The proposed fix — an allowlist of Google, Mozilla and Apple push origins — would break real users: Microsoft, Samsung, Brave and self-hosted UnifiedPush distributors all mint endpoints elsewhere, and the Web Push protocol (RFC 8030) makes the endpoint opaque to the application server on purpose. | `src/routes/push.ts` (`isValidEndpoint`), `src/notify/push-notifier.ts` | Accepted as low severity. Revisit only if a non-push-service host is ever observed in `push_subscriptions`, or if the Test button is ever exposed to a caller who is not the row's own owner |
...
| The reminder email's "I'm in" button renders solid/filled and "Can't make it" renders as an outline, so accepting reads as the default action. | **Superseded by M65, 10 September 2026.** The email now carries one button, "Respond on Make The Team", because the yes/no pair recorded nothing and sent players tapping twice — see `src/notify/templates/reminder.ts`. The earlier decision, kept for the record: **Settled, 11 August 2026 — keep it.** Raised as a product question because the product's value is an accurate count and a visual nudge toward accepting works against that. The owner's decision: encouraging people to play is the point, and the organiser's interest and the players' are aligned here. Not an open item; do not re-raise. |
