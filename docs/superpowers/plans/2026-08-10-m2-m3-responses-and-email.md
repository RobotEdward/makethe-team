...
This plan is a historical record of the instructions actually given to the implementers, so its body above is left as written, mistakes and all. The following turned out to be wrong once the tasks were carried out. They are recorded here, rather than edited into the tasks, so that whoever plans the next milestone does not rediscover them the hard way.

- **`fetchMock` from `cloudflare:test` does not exist in this pool version.** Task 12's dispatch (and this plan) told the implementer to test `ResendNotifier` against the network using `fetchMock` imported from `"cloudflare:test"`. The installed `@cloudflare/vitest-pool-workers@0.20.3` has no such export — the identifier appears nowhere in its type declarations. (The controller's own earlier read of that file mis-scanned leftover `MockInterceptor` namespace types as evidence it existed.) The implementer substituted `vi.spyOn(globalThis, "fetch")` with a throw-by-default spy, which is what every Resend test now uses.
- **`CRON_HOURLY_SWEEP` was renamed to `CRON_SWEEP` and its value changed to `"*/5 * * * *"`.** This plan's Task 16 (and the M0-M1 plan before it) named the constant `CRON_HOURLY_SWEEP` at an hourly `"0 * * * *"` cadence. For the testing phase, the owner asked for a 5-minute cadence to shorten reminder-pipeline test cycles; `CRON_HOURLY_SWEEP` would then be a lie about its own value, so it was renamed to `CRON_SWEEP` in `src/cron/handler.ts` alongside the matching `wrangler.jsonc` trigger change, with both sites commented as a temporary testing cadence to be reverted to hourly later. All references in `test/index.test.ts` and `test/cron/handler.test.ts` were updated to match.
- **`vi.mock` does not intercept a module's internal calls under this test pool.** This limits how call-ordering *inside* a function can be pinned from a test — a test wanting to prove, say, that `retirePastFixtures` runs after reminders are sent inside `handleScheduled` cannot mock one function and observe the other's call order from outside; it can only call the two functions directly and pin their independent behaviour. Task 16's ordering test does exactly that, and is honest about only pinning the functions, not the handler's call order.
- **workerd freezes `Date.now()` between I/O**, so a test isolate's clock and the Worker isolate's clock drift independently once any `await` crosses into the runtime. A test building an "expired" token as `Date.now() - 1000` flaked at roughly 20% (2 failures in 8 isolated runs) because a small negative margin could be erased by that drift before the token was even checked. The fix is to use absolute instants in tests — a fixed past or future `Date` — never a small relative margin computed from `Date.now()`.
