...
This plan is a historical record of the instructions actually given to the implementers, so its body above is left as written, mistakes and all. The following turned out to be wrong or incomplete once the tasks were carried out. They are recorded here, rather than edited into the tasks, so that whoever plans the next milestone does not rediscover them the hard way.

- **The `vitest.config.ts` and `tsconfig.json` in Task 1 were written against outdated package APIs.** The installed `@cloudflare/vitest-pool-workers@0.20.3` has removed the `./config` subpath shown above entirely. The working import is `cloudflareTest` (the Vite plugin) and `readD1Migrations`, both from the package root, used alongside `defineConfig` from `vitest/config` rather than a `defineWorkersConfig` wrapper. The `singleWorker` pool option shown in the plan's `poolOptions.workers` block no longer exists in this version. Separately, `@cloudflare/workers-types` v5 dropped dated subpaths, so the `"types"` entry referencing `@cloudflare/workers-types/2023-07-01` in the plan's `tsconfig.json` does not resolve either — the undated root import is what works now.
- **There is no `ProvidedEnv` type in this version of the pool.** Task 1's `test/env.d.ts` and Task 3's revision of it both declare `interface ProvidedEnv` inside `declare module "cloudflare:test"`. That type does not exist in the installed version. `env` imported from `cloudflare:test` is typed as `Cloudflare.Env`, so the correct augmentation is a global one — `declare global { namespace Cloudflare { interface Env extends Bindings { TEST_MIGRATIONS: D1Migration[] } } }` — not a `declare module "cloudflare:test"` one.
- **The local scheduled-event endpoint is `/cdn-cgi/local/scheduled`, not `/__scheduled`.** Wherever the plan's runbook text (Task 9's local verification steps) has the implementer hit a cron trigger by hand during `wrangler dev`, the path it gives is wrong. Wrangler's own dev startup banner prints the correct URL; use that rather than the path in this plan.
- **`wrangler deploy --dry-run` does not list cron triggers**, in wrangler 4.120.0, so the PR workflow's dry run (Task 1's `.github/workflows/pr.yml`) cannot be used to confirm the two cron schedules are configured — it simply doesn't show them either way. Confirming crons are registered requires a real deploy followed by a check against the Cloudflare API; both `0 * * * *` (the hourly sweep) and `15 3 * * *` (the daily materialisation) were confirmed present that way, not via dry run.
- **The WAF bot rule in the Cloudflare runbook (TR-37, `docs/runbooks/cloudflare.md`) was unusable as written and has been dropped.** It referenced the field `cf.client.bot_management.score`, which does not exist. The real field family, `cf.bot_management.*`, requires a paid Bot Management subscription that this Free Website zone does not have and is not getting. The rule is gone from the runbook; do not re-add a bot-scoring rule without first confirming the zone's plan actually exposes the fields it depends on.
- **D1 rejects any single statement with more than 100 bound parameters**, which the plan's schema and materialisation tasks (Task 3, Task 8) do not mention at all. It was found empirically while writing fixture inserts: 9 rows per statement worked, 10 failed with `D1_ERROR: too many SQL variables ... SQLITE_ERROR`. Fixture materialisation (`src/domain/materialise.ts`) now chunks inserts at 8 rows per statement rather than one statement per game. This has been written into the spec as TR-38 (§2.8, in the notes below the `games`/`fixtures` schema block) because it is not specific to materialisation: any future code that writes one row per squad member — most importantly the `pending` response rows §2.4/BR-1 requires for every eligible player when a fixture opens — will hit the same ceiling for a large enough squad and must chunk the same way.
