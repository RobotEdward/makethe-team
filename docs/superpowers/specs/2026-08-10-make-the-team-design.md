...
- **BR-18** Every player receives at most **one** unsolicited reminder (N-1) per fixture. Waitlist promotion (N-2), cancellation (N-3) and the owner attention email (N-4) are exempt: the first two are consequences of an action, and N-4 is separately capped at one per fixture by BR-31.
  - **Amended 2 September 2026 (M45).** Still one message, but its **wording depends on the reader's standing at send time**: a player already `in` is told "You're in." and offered only the way out, because asking "can you play?" of somebody who accepted weeks ago reads as though their answer went missing. Every other status gets the asking copy unchanged — including `out`, whose N-1 is the one prompt that lets a player whose plans changed get back in, and `waitlisted`, for whom "You're in." would simply be false. The count, the dedupe key and the administrator's kill switch are untouched. The "Can't make it" link is present in **both** wordings and must stay: every tier release and every waitlist promotion in the product is driven by an early dropout.
...
- **BR-35** An Owner may assign each `in` player of an `open` fixture to one of two sides. **Satisfied as of M9** ("team picking", the spec at `docs/superpowers/specs/2026-08-15-team-picking-design.md`). The sides are named per **Game** — `games.team_a_name` / `team_b_name`, defaulting to `Team A` and `Team B`, edited on the game form, blank falling back to the default — and an assignment is a nullable `responses.team` (`'a'`/`'b'`), which is already the (fixture, player) pair the assignment hangs off and so covers guests and dies with the response row. Only `in` players are offered a side: a waitlisted player has no place in the fixture yet, and the save route ignores any assignment naming one. **Saving and publishing are separate acts.** `POST /g/:id/f/:fixtureId/teams` writes the sides, stamps `fixtures.teams_saved_at`, clears `team` on every row that is no longer `in`, and tells nobody — a partial pick saves, because an Owner interrupted halfway must keep their work. `POST /g/:id/f/:fixtureId/teams/publish` sets `teams_published_at` — which nothing ever clears, so an announcement stays on record and the button never falls back to the label a never-published fixture shows — and sends N-9; it is refused, as the fixture page at 422 with the unassigned **named**, while anyone `in` has no side, and refused when nobody is `in` at all. Both routes are refused unless the fixture is still taking changes (the `takingChanges` predicate the row controls and the guest form already use), and both write `audit_log` rows (`fixture.teams_saved`, `fixture.teams_published`) per BR-27. The picker is a form of one radio group per player — three choices, the third being "not picked yet" — so it works with scripting off; the drag-and-drop enhancement only sets those same radios and never becomes required. **A player always sees their own side**, on `/r/:token`, on the player's game page and in the email, read from their own response row and never through `squadForViewer`; the other side's names follow BR-33, and a Game that hides its squad renders no line-ups rather than empty ones. **Nothing rebalances or re-sends itself.** No status change ever clears `responses.team`, so the orphaned value is the signal that the published sides no longer match the squad: a late drop-out or a waitlist promotion moves nobody, sends nothing, and surfaces to the Owner as teams needing another look, which only a further publish acts on. The Owner's next save is the one thing that clears it, which is how the signal is retired rather than latching on for the life of the fixture. An announcement is outstanding when `teams_published_at` is set and either `teams_saved_at` is later than it or one of those two roster conditions holds. Per-player ratings and algorithmic balancing remain future work (§308).

- **BR-36** An Owner may send a short message — subject up to 60 characters, body up to 500 — to a chosen audience of a Game or one of its Fixtures, on email, push, or both. **Satisfied as of M15** ("quick message to players", the spec at `docs/superpowers/specs/2026-08-18-quick-message-design.md`). The Game scope reaches `everyone` in the squad; the Fixture scope reaches one of the four response-derived audiences — `playing`, `waitlisted`, `pending` or `unavailable` — mapped from `responses.status` (`src/domain/broadcast-audience.ts`). Guests are never recipients on either scope, and neither is a player with no email and no registered push device, whatever audience they fall into (BR-32). A send is capped at `MAX_BROADCASTS_PER_GAME_PER_DAY` (three) per Game per UTC day, counted from `audit_log`'s `game.broadcast_sent` rows — the only record that a send happened, since there is no message table for players to read later. **A send whose chosen audience resolves to no addressable recipient is refused at 422 before the cap is charged** — it is not one of the Game's three for the day, so an organiser who mistypes an empty audience does not spend a send finding that out. A daily-ceiling refusal on the email leg (TR-31) deletes its `notification_log` row as every other deferral does, so `game.broadcast_email_deferred` records it durably, keyed on the **Game** — not a Fixture — because `entity_id` on the underlying broadcast is a game-scoped id whichever scope the message was composed from.
- **BR-37** After a fixture has been played, any player who was `in` it — and any active organiser of the game, whether or not they played — can say what happened: a score, or just who won. Everybody else in that electorate can agree with a claim, or file a different one. **Satisfied as of M25** ("recording the result", the spec at `docs/superpowers/specs/2026-08-21-fixture-results-design.md`). One claim per (fixture, player) — `fixture_result_claims`, `unique(fixture_id, player_id)` — so changing your mind updates your own row rather than adding a second one, and a candidate result is nothing but its backers grouped by outcome, then (among the scored claims for the winning outcome) by margin: an outcome-only claim of "Bibs won" counts toward the same outcome a scored "Bibs 3–2" does, which is why the tally has two levels and two separate confidence figures rather than one. Forty-eight hours after kickoff the most-backed claim locks (most backers, then a claim backed by an active organiser, then earliest filing) and becomes `fixture_results`, a cache of the derivation rather than a second source of truth — a fixture with nothing filed by the deadline stays writable, and the first claim filed after it locks immediately, alone, with no voting round. A fixture that has not been played answers 404 on every result route, distinct from the 422 a locked fixture's write gets: the first is an entitlement question, the second is not. `GET /g/:id/f/:fixtureId` dispatches by role (an owner gets the management page with the panel added, an active squad member gets the new player fixture page, anyone else 404s), and one write handler files, moves, agrees with, or clears a claim, because the form posts values, never a candidate id — there is nothing for a tampered submission to forge. Ratings, algorithmic balancing, league tables and any per-player statistic remain future work (§308) — this milestone records only the fact a rating would be fitted to.
...
**Amended, M15.** N-10 is added by BR-36 — the first notification a person
writes rather than the product. An Owner picks an audience (everyone in the
game, or one of a fixture's four response-derived slices), a subject and a
body, and either channel or both; the send is capped at
...
**Amended, M25.** N-12 is added by BR-37 — a nudge to file a result, sent
once full time has passed, to the same electorate BR-37 §6 lets file. **Push
is preferred and email is the fallback, the reverse of this table's usual
default (compare N-1, N-9, N-10, all email-first or email-and-push).** The
reason is TR-31: the daily send ceiling is email-only, N-12 is explicitly
low-urgency — nobody has to act on it, and the fixture page carries the same
ask indefinitely — while N-1 is the reminder this product cannot afford to
drop, so N-12 must not spend allowance N-1 may need. Selection is bounded by
a twelve-hour window from full time, not by "fixtures this sweep run
retired" — the sweep is hourly, so a fixture gets twelve chances to be
picked up, and a run missed for any reason costs nobody their nudge. The
email leg carries a BR-22 leave link scoped to `(gameId, playerId)`, matching
`send-teams.ts`'s own N-9 email; the push message carries none, matching
every other push notification in this table.
...
| **M10** | Design treatment — colour and type tokens with one meaning each, the answer-in-the-control pattern, and the destructive-page shape, applied to `layout.ts`, `fixture.ts`, `owner-fixture.ts` and `cancel.ts` | The four busiest screens share one visual language, and every token has a single documented meaning |
...
| **M25** | Recording the result (BR-37) — a claim table with no vote table to dangle from, a two-level tally (outcome, then margin), a 48-hour-from-kickoff lock with `fixture_results` as its recomputable cache, a new player fixture page, role-dispatched `GET /g/:id/f/:fixtureId`, and N-12 | A player who was in a played fixture records "Bibs 3–2" with JavaScript off, a second player agrees and the count reads 2, a third disagrees and both candidates show their backers, and 48 hours after kickoff the page shows the locked result with its two confidence figures and refuses every write at 422 |

M1–M4 are the product. M5–M7 make it shareable. Score recording was delivered as M25, as team picking was delivered as M9; the funding page remains a separate spec, still to come.
